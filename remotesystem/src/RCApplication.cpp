...
int RCApplication::addCb(int argc, char *argv[])
{
	...
	*/
	...
	if(mysql_query(thisPtr->db, query.str().c_str()) != 0)
	{
		DEBUG_MSG("[RCGateway::addCb] Insert location: Invalid query");	
		return -1;
	}
	...
	if(mysql_query(thisPtr->db, query.str().c_str()) != 0)
	{
		DEBUG_MSG("[RCGateway::addCb] Invalid locationID(" << locationID << ") or status(" << status << ")");	
		return -1;
	}
	...
	query << locationID << ",";
	query << "NULL" << ")"; // num vacants default = 0

	// execute query
	DEBUG_MSG("[RCGateway::addCb] " << query.str());
	if(mysql_query(thisPtr->db, query.str().c_str()) != 0)
	{
		DEBUG_MSG("[RCGateway::addCb] Insert parking_space: Invalid query");	
		return -1;
	}

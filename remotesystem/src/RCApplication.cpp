...
// Usage: ADDREG <post_code> <operator_id> <parish> <county> <district>
// Usage: ADDLOC <lat> <long> <post_code> <street>
...
int RCApplication::addCb(int argc, char *argv[])
{
	...
	*/
	...
	{
		DEBUG_MSG("[RCGateway::addCb] Invalid post_code(" << post_code << ")");	
		return -1;
	}
	...
	// INSERT lamppost: locationID, address, status
	{
		...
		{
			mysql_query(thisPtr->db, "ROLLBACK");
			DEBUG_MSG("[RCGateway::addCb] Invalid locationID(" << locationID << ") or address("<< address << ")");	
			return -1;
		}
	}
...
int RCApplication::consultCb(int argc, char *argv[])
{
    ...
    if(num_fields != 7)
    {
    	DEBUG_MSG("[RCApplication::consultCb] Invalid result with " << num_fields << " columns insted of 7");
  		err = -1;
    }
    else
    {

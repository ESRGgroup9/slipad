...
In order to light the streets efficiently, we use a LED lamp, that is the type of lamps with the better energy efficiency. It must be an adjustable lamp to control the lamp brightness, so the lamp selected was a G4 socket 12 V AC/DC LED lamp. The lamp is controlled using a driver, that is approached in subsection \ref{subsection:driver} the that takes a \ac{pwm} signal (Pin 18) from the Raspberry Pi as input. 
...
In a real-scale project, it would be used a different lamp, with supply voltage of 230 V AC and other specific characteristics  like  rate, in  from water and dust (\ac{ipprotect}65 \ref{ip65}). It would be also used a LED lamp with higher electric power. 

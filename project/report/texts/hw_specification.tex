...
In order to know when the lamp is broken, one needs to use a lamp failure detector, so it was designed a circuit represented in figure \ref{fig:lampFailure}. 
...
This circuit \[V_{+} = V_{cc} . \dfrac{R_2}{R_1 + R_2} \]
...
\begin{table}[H]
	...
	\begin{tabular}{|m{5cm}|m{6cm}|}
		...
		AmpOp & LM358
		\\\hline
		Resistor \(R_{1}\) & xx $\Omega$; +-5 \%
		\\\hline
		Resistor \(R_{2}\) & yy $\Omega$; +-5 \%
		\\\hline
